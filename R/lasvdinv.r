...
lasvdFixvarinv <- function(design, resp, xi, nstarts, nmc, n0, nn, noiseVar,
                           ...
                           kersigfrac=.05)
{
    ...
    poststarts <- logpost[idx]
    nsample <- floor((nmc-nburn)/nthin)
    out <- .C("lagpFixvarInv",as.integer(ndesign), as.integer(nparam),
              as.integer(nstarts), as.integer(nmc), as.integer(nburn),
              as.integer(nthin), as.integer(tlen), as.integer(n0),
              as.integer(nn), as.integer(nfea), as.integer(resvdThres),
              as.integer(every), as.integer(nthread), as.double(noiseVar),
...
lasvdProfileAdaptiveinv <- function(design, resp, xi, nstarts, nmc, n0, nn,
                                    nfea = min(1000,nrow(design)),
                                    nsvd=nn, nburn=0, nthin=1,resvdThres = min(5, nn-n0),
                                    every = min(5,nn-n0), frac = .95, gstart = 0.0001,
                                    ...
                                    kersigfrac=.05)
{
    ...
    poststarts <- logpost[idx]
    nsample <- floor((nmc-nburn)/nthin)
    out <- .C("lagpProfileAdaptiveInv",as.integer(ndesign), as.integer(nparam),
              as.integer(nstarts), as.integer(nmc), as.integer(nburn),
              as.integer(nthin), as.integer(tlen), as.integer(n0),
              as.integer(nn), as.integer(nfea), as.integer(resvdThres),
              as.integer(every), as.integer(nthread), as.integer(adpthres),as.double(frac),
              as.double(gstart), as.double(kersd), as.double(xi),
              as.double(t(design)), as.double(resp), as.double(t(xstarts)),
              as.double(poststarts), as.double(lb), as.double(ub),as.double(eps),
              as.double(sval), samples=double(nstarts*nparam*nsample))
    samples <- matrix(out$samples,nrow=nstarts*nsample,byrow=TRUE)
    return(samples)
}

lasvdFixvarAdaptiveinv <- function(design, resp, xi, nstarts, nmc, n0, nn, noiseVar,
                                   nfea = min(1000,nrow(design)),
                                   nsvd=nn, nburn=0, nthin=1,resvdThres = min(5, nn-n0),
                                   every = min(5,nn-n0), frac = .95, gstart = 0.0001,
                                   nthread = 4, adpthres=100*ncol(design), eps=1e-5, sval=5.76/ncol(design)^2,
                                   ...
                                   kersigfrac=.05)
{
    ...
    poststarts <- logpost[idx]
    nsample <- floor((nmc-nburn)/nthin)
    out <- .C("lagpFixvarAdaptiveInv",as.integer(ndesign), as.integer(nparam),
              as.integer(nstarts), as.integer(nmc), as.integer(nburn),
              as.integer(nthin), as.integer(tlen), as.integer(n0),
              as.integer(nn), as.integer(nfea), as.integer(resvdThres),
              ...
              as.double(t(design)), as.double(resp), as.double(t(xstarts)),
              as.double(poststarts), as.double(lb), as.double(ub),
              as.double(eps), as.double(sval), samples=double(nstarts*nparam*nsample))
    samples <- matrix(out$samples,nrow=nstarts*nsample,byrow=TRUE)
    return(samples)
}
